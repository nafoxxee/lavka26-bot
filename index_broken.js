...
const mainMenu = Markup.keyboard([
    ['📄 Смотреть объявления'],
    ['➕ Создать объявление', '📢 Рекламное объявление'],
    ['❤️ Избранное', '🔍 Поиск'],
    ['⚙ Настройки']
]).resize();
...
bot.on('photo', async (ctx) => {
    ...
    if (state.step === 'payment_screenshot') {
        ...
        try {
            ...
            if (error || !ad) {
                await ctx.reply('❌ Ошибка поиска объявления');
                return;
            }
            
            // Отправляем уведомление админу о скриншоте оплаты
            await notifyAdminAboutPaymentScreenshot(userId, adId, fileId, ad);
            
            await ctx.reply(
                '✅ Скриншот отправлен на проверку!\n\n' +
                '🔔 Объявление будет опубликовано в течение 15 минут\n' +
            ...
            console.error('Ошибка обработки скриншота оплаты:', error);
            await ctx.reply('❌ Ошибка отправки скриншота. Попробуйте еще раз.');
        }
        return;
    }

    if (state.step === 'photo') {
        // Обработка фото для обычных объявлений
        const fileId = ctx.message.photo[ctx.message.photo.length - 1].file_id;
        ...
        await ctx.reply(`📸 Фото добавлено (${state.data.photos.length}/10)`);
    } else if (state.step === 'ad_media') {
        // Обработка фото для рекламных объявлений
        const fileId = ctx.message.photo[ctx.message.photo.length - 1].file_id;
        if (!state.data.photos) {
            state.data.photos = [];
        }
...
// Обработка видео для рекламных объявлений
bot.on('video', async (ctx) => {
    const userId = ctx.from.id;
    const state = userStates.get(userId);

    // Обработка для рекламных объявлений
    if (state && state.step === 'ad_media') {
        if (!state.data.videos) state.data.videos = [];
        
        const videoId = ctx.message.video.file_id;
        state.data.videos.push(videoId);

        const totalMedia = (state.data.photos?.length || 0) + (state.data.videos?.length || 0);
        
        if (totalMedia >= 10) {
            await ctx.reply(
                `📸 Медиа добавлено (${totalMedia}/10) - лимит достигнут\n` +
            ...
            await ctx.reply(
                `📸 Медиа добавлено (${totalMedia}/10)\n` +
                'Добавьте еще фото/видео или нажмите "Далее"',
                Markup.keyboard([
                    ['Далее', '❌ Отмена']
                ]).resize()
            );
...
// Команда "Далее" - для первого шага и рекламных объявлений
bot.hears('Далее', async (ctx) => {
    const userId = ctx.from.id;
    const state = userStates.get(userId);
    ...
    if (state.step === 'photo') {
        ...
        await ctx.reply(
            ...
            'Шаг 6 из 6: 📞 Связь\n\n' +
            'Введите контактную информацию (телефон, email, Telegram):',
            Markup.keyboard([
                ['❌ Отмена']
            ]).resize()
        );
    } else if (state.step === 'location') {
        // Пропускаем геолокацию для обычных объявлений
        console.log('Пропуск геолокации, переходим к публикации');
        await publishAd(ctx, await getOrCreateUser(ctx), state.data);
        userStates.delete(userId);
    }
...
// Команда для ввода адреса вручную
bot.hears('✍️ Ввести адрес вручную', async (ctx) => {
    const userId = ctx.from.id;
    const state = userStates.get(userId);
    ...
    if (state.step === 'ad_location' || state.step === 'location') {
        state.step = 'manual_address';
        await ctx.reply(
            '📍 Ввод адреса вручную\n\n' +
            'Введите адрес в строгом формате:\n\n' +
            '🏙️ Город, Улица, Дом\n\n' +
            'Примеры:\n' +
            '• Москва, Тверская улица, 1\n' +
            '• Санкт-Петербург, Невский проспект, 25\n' +
            '• Казань, Баумана улица, 5\n\n' +
            '⚠️ Обязательно используйте запятые как разделители!',
            Markup.keyboard([
                ['❌ Отмена']
            ]).resize()
        );
...
bot.on('location', async (ctx) => {
    ...
    // Обработка для обычных объявлений
    ...
    // Обработка для рекламных объявлений
    if (state && state.step === 'ad_location') {
        state.data.location = {
            latitude: ctx.message.location.latitude,
            longitude: ctx.message.location.longitude
        };
...
async function publishPromoAd(ctx, user, adData) {
    try {
        console.log('Начинаем публикацию рекламного объявления');
        console.log('Данные объявления:', adData);

        // Проверяем обязательные поля
        if (!adData.title || !adData.description || !adData.contact) {
        ...
        // Ищем или создаем категорию для рекламных объявлений
        let categoryId = 15; // по умолчанию
        ...
        if (error) {
            console.error('Ошибка при вставке рекламного объявления:', error);
            await ctx.reply(`❌ Ошибка при публикации: ${error.message}`);
            return;
        }

        console.log('Рекламное объявление успешно создано:', ad);
...
async function notifyAdminAboutNewPromoAd(ad) {
    try {
        ...
        let adminText = `🔔 Новое РЕКЛАМНОЕ объявление на модерации!\n\n`;
        adminText += `📢 ${ad.title}\n`;
        adminText += `${ad.description}\n\n`;
        adminText += `💰 Цена: ${ad.price || 'Не указана'} ₽\n`;
        adminText += `🌐 Сайт: ${ad.website || 'Не указан'}\n`;
        adminText += `📞 Контакт: ${ad.contact}\n`;
        ...
        console.error('Ошибка отправки уведомления админу о рекламе:', error);
    }
}
...
async function sendLimitNotification(ctx, limitInfo) {
    const limitText = `📊 Лимит бесплатных объявлений\n\n` +
        ...
        `• Создать платное объявление - ${AD_PRICE} ₽\n` +
        `• Ожидать сброса лимита (ежемесячно)\n\n` +
        `🚀 Узнать о продвижении → /promotion`;

    await ctx.reply(limitText, Markup.inlineKeyboard([
        [Markup.button.callback(`💳 Создать за ${AD_PRICE} ₽`, 'create_paid_ad')],
        [Markup.button.callback('⬅ Назад', 'back_to_main')]
    ]));
}

// Уведомление пользователя об одобрении рекламного объявления
async function notifyUserAboutPromoApproved(userId, adId, ad) {
    try {
        ...
        try {
            ...
            if (settings) {
                sbpPhone = settings.sbp_phone;
                sbpBank = settings.sbp_bank;
            }
        } catch (error) {
            console.log('Ошибка получения настроек СБП:', error.message);
        }
        
        // Если нет в БД, пробуем временные настройки
        if (!sbpPhone && global.tempSettings) {
            sbpPhone = global.tempSettings.sbp_phone;
            sbpBank = global.tempSettings.sbp_bank;
        }

        let notificationText = `✅ Ваше рекламное объявление одобрено!\n\n` +
            `📢 ${ad.title}\n\n` +
            `💰 Стоимость размещения: ${PROMO_AD_PRICE} ₽\n\n` +
            `🔔 Размещение рекламных объявлений платное\n\n`;
            
        if (sbpPhone && sbpBank) {
            notificationText += `💳 Реквизиты для оплаты:\n` +
                `📱 Телефон: ${sbpPhone}\n` +
                `🏦 Банк: ${sbpBank}\n\n` +
                `💳 После оплаты нажмите кнопку "Оплатил"`;
        } else {
            notificationText += `⚠️ Реквизиты СБП не настроены\n` +
                `📞 Свяжитесь с администратором`;
        }

        await bot.telegram.sendMessage(userId, notificationText, Markup.inlineKeyboard([
            [Markup.button.callback('💳 Оплатить', `pay_promo_${adId}`)],
            [Markup.button.callback('🏠 Главное меню', 'back_to_main')]
        ]));
    } catch (error) {
        console.error('Ошибка уведомления об одобрении рекламы:', error);
    }
}
...
async function notifyUserAboutModeration(userId, adId, status, reason = null) {
    try {
        ...
        if (status === 'active') {
            notificationText = `✅ Ваше объявление одобрено!\n\n📝 ${ad.title}\n\n🎉 Объявление опубликовано в канале @lavka26city`;
...
bot.hears('📢 Рекламное объявление', async (ctx) => {
    const userId = ctx.from.id;
    
    // Инициализация состояния для создания рекламного объявления
    userStates.set(userId, {
        step: 'ad_title',
        data: {}
    });
...
bot.hears('💰 Финансы', async (ctx) => {
    if (ctx.from.id !== ADMIN_ID) {
        await ctx.reply('❌ Эта функция доступна только администратору');
        return;
    }
...
async function showModerationAd(ctx, ads, index) {
    ...
    const isPromo = ad.is_promo;
    const adType = isPromo ? '📢 РЕКЛАМНОЕ' : '📝 Обычное';
    
    let text = `${adType} объявление ${index + 1}/${ads.length} на модерации\n\n`;
    text += `📝 ${ad.title}\n\n`;
    text += `${ad.description}\n\n`;
    
    if (isPromo) {
        // Дополнительные поля для рекламных объявлений
        if (ad.website) text += `🌐 Сайт: ${ad.website}\n`;
        if (ad.contact) text += `📞 Контакт: ${ad.contact}\n`;
        text += `💰 Цена: ${ad.price || 'Не указана'} ₽\n`;
    } else {
        text += `💰 ${ad.price} ₽\n`;
    }
    ...
    switch (state.step) {
        case 'finance_phone':
            ...
            try {
                ...
                if (updateError) {
                    console.log('Ошибка обновления телефона:', updateError.message);
                    
                    // Если таблица не существует, создаем временное хранилище
                    if (!global.tempSettings) {
                        global.tempSettings = {};
                    }
                    global.tempSettings.sbp_phone = phone;
                    
                    await ctx.reply(
                        '⚠️ Телефон сохранен временно\n\n' +
                        `📱 Телефон: ${phone}\n\n` +
                        '⚠️ Для постоянного сохранения нужно создать таблицу settings в Supabase',
                        Markup.inlineKeyboard([
                            [Markup.button.callback('⬅ Назад', 'finance_sbp')]
                        ])
                    );
                } else {
                    await ctx.reply(
                        '✅ Телефон СБП успешно обновлен!\n\n' +
                        `📱 Новый телефон: ${phone}`,
                        Markup.inlineKeyboard([
                            [Markup.button.callback('⬅ Назад', 'finance_sbp')]
                        ])
        ...
        case 'finance_bank':
            ...
            try {
                ...
                if (updateError) {
                    console.log('Ошибка обновления банка:', updateError.message);
                    
                    // Если таблица не существует, создаем временное хранилище
                    if (!global.tempSettings) {
                        global.tempSettings = {};
                    }
                    global.tempSettings.sbp_bank = text.trim();
                    
                    await ctx.reply(
                        '⚠️ Банк сохранен временно\n\n' +
                        `🏦 Банк: ${text.trim()}\n\n` +
                        '⚠️ Для постоянного сохранения нужно создать таблицу settings в Supabase',
                        Markup.inlineKeyboard([
                            [Markup.button.callback('⬅ Назад', 'finance_sbp')]
                    ...
                    await ctx.reply(
                        '✅ Банк СБП успешно обновлен!\n\n' +
                        `🏦 Новый банк: ${text.trim()}`,
                        Markup.inlineKeyboard([
                            [Markup.button.callback('⬅ Назад', 'finance_sbp')]
                        ])
                    );
        ...
        case 'ad_location':
            if (text === 'Пропустить') {
                ...
                state.data.location_text = text.trim();
            }
            state.step = 'ad_media';
            await ctx.reply(
                '📢 Создание рекламного объявления\n\n' +
        ...
        case 'ad_contact':
            ...
            await ctx.reply(
                '📢 Создание рекламного объявления\n\n' +
                'Шаг 6 из 6: 🕵️️ Настройки приватности\n\n' +
...
bot.on('callback_query', async (ctx) => {
  ...
  if (action === 'back_to_main') {
    ...
    try {
        ...
        await ctx.reply('Выберите действие в меню ниже:');
    }
    ...
    try {
        if (actionType === 'approve') {
            // Получаем объявление для проверки типа
            ...
            if (fetchError || !ad) {
                console.error('Ошибка получения объявления:', fetchError);
                await ctx.answerCbQuery('❌ Ошибка получения объявления');
                return;
            }
            ...
            if (ad.is_promo) {
                // Для рекламных объявлений - отправляем уведомление об оплате
                try {
                    await ctx.editMessageText(
                        ...
                        `🔔 Размещение рекламных объявлений платное`,
                        Markup.inlineKeyboard([
                            [Markup.button.callback('⬅ Далее', `moderate_skip_${index}`)]
                        ])
                    );
                } catch (editError) {
                    // Если не удалось редактировать, отправляем новое сообщение
                ...
                // Отправляем уведомление пользователю с просьбой оплатить
                await notifyUserAboutPromoApproved(ad.user_id, adId, ad);
            } else {
                // Для обычных объявлений - сразу отправляем в канал
                if (CHANNEL_ID) {
                ...
                // Показываем следующее объявление
                await showModerationAd(ctx, ads, index + 1);
            }
  ...
  // Обработка платных функций
  if (action === 'create_paid_ad') {
    const userId = ctx.from.id;
    
    // Инициализация состояния для создания платного объявления
    userStates.set(userId, {
      step: 'photo',
      data: { isPaid: true }
    });
    ...
    await ctx.editMessageText(
      '💳 Создание платного объявления\n\n' +
      `Стоимость: ${AD_PRICE} ₽\n\n` +
      '📸 Шаг 1: Отправьте фото (можно несколько)\n' +
      'Когда закончите, нажмите кнопку "Далее"',
      Markup.keyboard([
        ['Далее', '❌ Отмена']
      ]).resize()
    ...
    await ctx.answerCbQuery();
  } else if (action === 'create_paid_promo') {
    const userId = ctx.from.id;
    
    // Инициализация состояния для создания платного рекламного объявления
    userStates.set(userId, {
      step: 'ad_title',
      data: { isPaid: true }
    });
    ...
    await ctx.editMessageText(
      '💳 Создание платного рекламного объявления\n\n' +
      `Стоимость: ${PROMO_AD_PRICE} ₽\n\n` +
      '📝 Шаг 1: Введите название объявления (3-100 символов):',
      Markup.keyboard([
        ['❌ Отмена']
      ]).resize()
    );
    ...
    // Обработка продвижения объявлений
    const parts = action.split('_');
    const adId = parts[1];
    ...
    try {
      ...
      const price = PROMOTION_PRICES[`boost_${duration}`] || 0;
      
      await ctx.editMessageText(
        `🚀 Продвижение объявления\n\n` +
        `📝 ${ad.title}\n` +
        `💰 Стоимость: ${price} ₽\n\n` +
        `⭐ VIP на ${duration === 'day' ? '1 день' : duration === 'week' ? '7 дней' : 'месяц'}\n\n` +
        `💳 Для оплаты свяжитесь с администратором\n` +
        `📞 @${ADMIN_ID}\n\n` +
        `После оплаты ваше объявление будет:\n` +
        `• Закреплено вверху канала\n` +
        `• Выделено специальным цветом`,
        Markup.inlineKeyboard([
          [Markup.button.callback('⬅ Назад', 'back_to_main')]
        ])
      );
      ...
      console.error('Ошибка при обработке продвижения:', error);
      await ctx.answerCbQuery('❌ Ошибка');
    }
  }
  
  // Обработка оплаты рекламного объявления
  if (action.startsWith('pay_promo_')) {
    ...
    if (sbpPhone && sbpBank) {
        ...
        const sbpUrl = `https://qr.nspk.ru/AS2/BR/${sbpPhone}?bank=${sbpBank}&sum=${PROMO_AD_PRICE}.00&currency=RUB&comment=Рекламное объявление Lavka26`;
        
        await ctx.editMessageText(
            `💳 Оплата рекламного объявления\n\n` +
            `💰 Сумма: ${PROMO_AD_PRICE} ₽\n` +
            `📱 Получатель: ${sbpPhone}\n` +
            `🏦 Банк: ${sbpBank}\n\n` +
            `🔗 Нажмите ниже для оплаты в банке`,
            Markup.inlineKeyboard([
                [Markup.button.url('💳 Оплатить в банке', sbpUrl)],
                [Markup.button.callback('✅ Оплатил', `paid_promo_${adId}`)],
                [Markup.button.callback('⬅ Назад', 'back_to_main')]
            ])
        );
    } else {
        await ctx.answerCbQuery('⚠️ Реквизиты СБП не настроены');
    }
    
    await ctx.answerCbQuery();
  }
  
  // Обработка статуса "Оплатил"
  if (action.startsWith('paid_promo_')) {
    ...
    await ctx.editMessageText(
      ...
      '💰 Сделайте скриншот экрана с подтверждением оплаты\n\n' +
  ...
  // Обработка отправки скриншота оплаты
  if (action.startsWith('send_payment_screenshot_')) {
    const adId = action.replace('send_payment_screenshot_', '');
  ...
  // Обработка отклонения оплаты
  if (action.startsWith('reject_payment_')) {
    if (ctx.from.id !== ADMIN_ID) {
    ...
    try {
      ...
      await bot.telegram.sendMessage(userId, 
        '❌ Оплата отклонена\n\n' +
        '📄 Скриншот оплаты не прошел проверку\n' +
        '💡 Пожалуйста, проверьте реквизиты и отправьте новый скриншот\n' +
        '📞 Если есть вопросы, свяжитесь с администратором',
        Markup.inlineKeyboard([
          [Markup.button.callback('📸 Отправить новый скриншот', `send_payment_screenshot_${adId}`)],
          [Markup.button.callback('🏠 Главное меню', 'back_to_main')]
        ])
      );
      ...
      await ctx.editMessageText(
        '❌ Оплата отклонена\n\n' +
        '📄 Пользователь уведомлен о проблеме с оплатой',
        Markup.inlineKeyboard([
          [Markup.button.callback('⬅ Назад', 'back_to_main')]
        ])
      );
  ...
  // Обработка финансовых функций
  if (action === 'finance_sbp') {
    try {
      // Получаем текущие реквизиты СБП
      ...
      let sbpText = '💰 Реквизиты СБП\n\n';
      
      if (settings && settings.sbp_phone) {
        sbpText += `📱 Телефон: ${settings.sbp_phone}\n`;
        sbpText += `🏦 Банк: ${settings.sbp_bank || global.tempSettings?.sbp_bank || 'Не указан'}\n\n`;
      } else if (global.tempSettings) {
        sbpText += `📱 Телефон: ${global.tempSettings.sbp_phone || 'Не указан'}\n`;
        sbpText += `🏦 Банк: ${global.tempSettings.sbp_bank || 'Не указан'}\n\n`;
        sbpText += '⚠️ Данные сохранены временно\n\n';
      } else {
        sbpText += 'Реквизиты еще не настроены\n\n';
      }

      sbpText += 'Введите новые реквизиты в формате:\n';
      sbpText += 'Телефон: 79123456789\n';
      sbpText += 'Банк: Сбер\n\n';
      sbpText += 'Или отправьте команду для изменения:';

      await ctx.editMessageText(sbpText, Markup.inlineKeyboard([
        [Markup.button.callback('📝 Изменить телефон', 'finance_edit_phone')],
        [Markup.button.callback('🏦 Изменить банк', 'finance_edit_bank')],
        [Markup.button.callback('⬅ Назад', 'back_to_main')]
      ]));
    } catch (error) {
      console.error('Ошибка загрузки реквизитов СБП:', error);
      await ctx.editMessageText('❌ Ошибка загрузки реквизитов');
    }
  } else if (action === 'finance_edit_phone') {
    userStates.set(userId, { step: 'finance_phone', data: {} });
    await ctx.editMessageText(
  ...
  } else if (action === 'finance_edit_bank') {
    userStates.set(userId, { step: 'finance_bank', data: {} });
    await ctx.editMessageText(
      '💰 Изменение банка СБП\n\n' +
    ...
    try {
      ...
      statsText += `📄 Всего объявлений: ${totalAds}\n`;
